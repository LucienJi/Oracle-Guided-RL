...
class CurrMaxAdv(BaseAlgo):
    ...
    @torch.no_grad()
    def _get_oracle_training_critic(self, obs_t: torch.Tensor, actions: torch.Tensor, oracle_index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        oracle_name = self.oracles_names[int(oracle_index)]
    ...
    def score_action_obs(self, obs_rep: torch.Tensor, actions: torch.Tensor, action_candidates_index: torch.Tensor, if_greedy: bool = False):
        ...
        """
        q_mean, q_std = self._get_learner_training_critic(obs_rep, actions)  # (N,)
    ...
    def _compute_learner_actor_loss(self, data: dict):
        ...
        with torch.no_grad():
            # Q_max(s, a_data) across critics
            obs = data["observations"]
            obs_raw = data.get("observations_raw", obs)
            qs = self.learner_critic.get_value(obs, actions).squeeze(-1)  # (num_qs, B)
            q_max = qs.mean(dim=0)  # (B,)

            # V_max(s) estimated from critics using k=20 action samples from each policy.
            k = 20
            b = obs.shape[0]
            obs_rep = obs.unsqueeze(0).expand(k, b, -1).reshape(k * b, -1)  # (k*B, D)

            a_k = self.learner_actor.sample_action(obs, self.target_policy_noise, k)  # (k, B, A)
            q_pi = self.learner_critic.get_value(obs_rep, a_k.reshape(k * b, -1)).squeeze(-1)  # (num_qs, k*B)
            v_max = q_pi.reshape(q_pi.shape[0], k, b).mean(dim=1).mean(dim=0)  # (B,)

            for name in self.oracles_names:
                # Q_max(s, a_data)
                qk = self.oracles_critics[name].get_value(obs, actions).squeeze(-1).mean(dim=0)
                q_max = torch.maximum(q_max, qk)
                # V(s) from oracle policy via its critic
                std = float(self.oracle_std_multiplier_by_name[name])
                a_k = self.oracles_actors[name].sample_action(obs_raw, std, k)  # (k, B, A)
                q_pi = self.oracles_critics[name].get_value(obs_rep, a_k.reshape(k * b, -1)).squeeze(-1)
                v_k = q_pi.reshape(q_pi.shape[0], k, b).mean(dim=1).mean(dim=0)
                v_max = torch.maximum(v_max, v_k)
