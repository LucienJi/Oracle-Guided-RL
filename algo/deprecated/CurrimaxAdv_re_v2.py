...
class CurrMaxAdv(BaseAlgo):
    def __init__(
        ...
        assert len(self.oracles_names) == len(self.oracles_critics) == self.n_oracles
        ...
        # No separate value networks: estimate V(s) by sampling K actions from policy and averaging Q(s,a)
        self.value_action_samples_k = int(self.args.get("value_action_samples_k", 10))
        self.value_action_samples_k = max(1, self.value_action_samples_k)
    ...
    @torch.no_grad()
    def _estimate_v_from_policy(
        self,
        obs_t: torch.Tensor,
        *,
        actor: OraclePolicyBase,
        critic: SimbaCritics,
        noise_std: float,
        k: int,
    ) -> torch.Tensor:
        """
        Approximate V(s) without a value network:
          - sample K actions from the policy
          - evaluate Q(s,a) under the critic
          - return mean_a Q(s,a)

        Returns: (num_qs, B)  (ensemble dimension preserved; average over K sampled actions)
        """
        k = max(1, int(k))
        b = int(obs_t.shape[0])
        ...
        if k == 1:
            actions_k = actor.get_action(obs_t, float(noise_std)).unsqueeze(0)  # (1, B, A)
        else:
            actions_k = actor.sample_action(obs_t, float(noise_std), k)  # (k, B, A)
        actions_flat = self._flatten_action_candidates(actions_k)  # (k*B, A)
        obs_rep = obs_t.unsqueeze(0).expand(k, b, -1).reshape(k * b, -1)  # (k*B, D)
        qs = critic.get_value(obs_rep, actions_flat)  # (num_qs, k*B, 1)
        qs = qs.squeeze(-1)  # (num_qs, k*B)
        n = int(qs.shape[0])
        return qs.reshape(n, k, b).mean(dim=1)  # (num_qs, B)
    # -----------------------
    ...
    @torch.no_grad()
    def _get_learner_training_critic(self, obs_t: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        qs = self.learner_critic.get_value(obs_t, actions)  # (num_qs, B, 1)
        min_qs = qs.min(dim=0).values.squeeze(-1)
        return min_qs

    @torch.no_grad()
    def _get_oracle_training_critic(self, obs_t: torch.Tensor, actions: torch.Tensor, oracle_index: int) -> torch.Tensor:
        oracle_name = self.oracles_names[int(oracle_index)]
    ...
    def score_action_obs(self, obs_rep: torch.Tensor, actions: torch.Tensor, action_candidates_index: torch.Tensor, if_greedy: bool = False):
        ...
        q = self._get_learner_training_critic(obs_rep, actions)  # (N,)        
        effective_kappa = 1e10 if if_greedy else 0.0
        other_mask = (action_candidates_index != 0).float()  # (N,) 1.0 for non-learner actions
        adjusted_q = q - effective_kappa * other_mask  # (N,)
        
        for i in range(self.n_oracles):
            q = self._get_oracle_training_critic(obs_rep, actions, i)  # (N,)
            other_mask = (action_candidates_index != (i + 1)).float()  # (N,) 1.0 for non-oracle-i actions
            q_orc = q - effective_kappa * other_mask
    ...
    def get_guided_training_action(self,raw_obs):
        ...
        policy_index = sampling_infos["env/selected_action_index"]// max(k, 1)
    ...
    def update(self):
        ...
        learner_data = self._prepare_sampled_batch(self.replay_buffer.action_mask_sample(int(self.args["learner_critic_batch_size"]), 0,on_policy_ratio=0.5))
        learner_mini_batches = reshape_into_blocks(learner_data, num_blocks)

        oracle_mini_batches = {}
        for oracle_idx, name in enumerate(self.oracles_names):
            odata = self._prepare_sampled_batch(self.replay_buffer.action_mask_sample(int(self.args["learner_critic_batch_size"]), oracle_idx + 1,on_policy_ratio=0.5))
    ...
    def update_critic(self, learner_data: dict, oracle_data_by_name: Optional[Dict[str, dict]] = None):
        ...
        q_loss, info = self._compute_critic_loss(learner_data, oracle_data_by_name)
    ...
    def _compute_critic_loss(self, learner_data: dict, oracle_data_by_name: Optional[Dict[str, dict]] = None):
        ...
        ## oracles part
        for name in self.oracles_names:
            q_loss, q_mean, q_uncertainty = self._compiled_compute_oracles_critic_loss(oracle_data_by_name[name], name)
            total_q_loss += q_loss
            info.update(
                    {
    ...
    def _compute_oracles_critic_loss(self, data: dict,oracle_name:str):
        ...
        with torch.no_grad():
            ...
            oracle_action = self.oracles_actors[oracle_name].get_action(data["next_observations"], std)
        ...
        for _i, pred_info in enumerate(pred_q_infos):
            ...
            q_loss_candidates.append(loss)
        q_loss_candidates = torch.stack(q_loss_candidates, dim=0)
        N, B = q_loss_candidates.shape
        p = 0.6
        # (N_ensemble, B)
        ensemble_masks = torch.bernoulli(
            torch.full((N, B), p, device=self.device)
        ).bool()
        masked_losses = q_loss_candidates * ensemble_masks.float()   # (N, B)
        denom = ensemble_masks.sum(dim=1).clamp(min=1)               # (N,)
        loss_per_ensemble = masked_losses.sum(dim=1) / denom         # (N,)
        q_loss = loss_per_ensemble.mean()
        
        q_mean = pred_qs.mean()
        q_uncertainty = pred_qs.std(dim=0).mean()
        
        return q_loss, q_mean, q_uncertainty.mean()
    ...
    def _compute_learner_actor_loss(self, data: dict):
        ...
        with torch.no_grad():
            ...
            q_max = qs.min(0).values.squeeze(-1)  # (B,)
            for critic in self._oracle_critic_list:
                qk = critic.get_value(data["observations"], actions).squeeze(-1).min(dim=0).values
                q_max = torch.maximum(q_max, qk)

            # V_max(s) approximation (no value networks):
            # For each policy, sample K actions and compute mean_a Q(s,a) (ensemble preserved),
            # then reduce across ensemble and take max across policies.
            k = int(self.value_action_samples_k)
            v_learner_ens = self._estimate_v_from_policy(
                data["observations"],
                actor=self.learner_actor,
                critic=self.learner_critic,
                noise_std=float(self.task_specific_noise),
                k=k,
            )  # (num_qs, B)
            v_max = v_learner_ens.min(dim=0).values  # (B,)
            for oracle_name in self.oracles_names:
                v_oracle_ens = self._estimate_v_from_policy(
                    data["observations"],
                    actor=self.oracles_actors[oracle_name],
                    critic=self.oracles_critics[oracle_name],
                    noise_std=float(self.oracle_std_multiplier_by_name[oracle_name]),
                    k=k,
                )  # (num_qs, B)
                v_max = torch.maximum(v_max, v_oracle_ens.min(dim=0).values)
            advantage = q_max - v_max

            pos_mask = (advantage > 0)  # (B,)
            pos_mask_f = pos_mask.to(dtype=advantage.dtype)
            positive_ratio = pos_mask_f.mean()
            # Only weight samples with positive advantage
        ...
        raw_loss = (predicted_action - data["actions"]).abs().sum(dim=-1) # Using L1 loss instead of L2 loss, robust to outlier weak oracles
