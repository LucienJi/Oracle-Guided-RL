...
class MaxAdv(BaseAlgo):
    def __init__(
        ...
        if self.normalize_observations:
            ...
            # Back-compat key used by checkpoint payloads
        ...
        if self.use_compile and hasattr(torch, "compile"):
            ...
            self._get_learner_training_critic = self._try_compile(
                self._get_learner_training_critic, "get_learner_training_critic", fullgraph=False
            )
            self._get_oracle_training_critic = self._try_compile(
                self._get_oracle_training_critic, "get_oracle_training_critic", fullgraph=False
            )
        ...
        print(f"MaxAdv: compile={'on' if self.use_compile else 'off'}")
    ...
    def _get_oracle_training_critic(self, obs_t: torch.Tensor, actions: torch.Tensor, oracle_index: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        vs = self.oracles_values[oracle_name].get_value(obs_t)  # (num_qs, B, 1)
        v = vs.mean(dim=0).squeeze(-1)
        q_mean = qs.mean(dim=0).squeeze(-1)
        q_std = qs.std(dim=0).squeeze(-1)
        return q_mean, q_std, v
    ...
    def score_action_obs(self, obs_rep: torch.Tensor, actions: torch.Tensor, action_candidates_index: torch.Tensor, if_greedy: bool = False):
        ...
        for i in range(self.n_oracles):
            q_mean, q_std, v = self._get_oracle_training_critic(obs_rep, actions, i)  # (N,)
    ...
    def _compute_oracles_critic_loss(self, data: dict,oracle_name:str):
        ...
        with torch.no_grad():
            std = float(self.oracle_std_multiplier_by_name[oracle_name])
            actions = self.oracles_actors[oracle_name].get_action(data["observations"], std)
        ...
        #     oracle_critic_values = self.oracles_critics[oracle_name].get_target_value(
        #         data["observations"], actions
        #     )  # shape = (num_qs, B, 1)
        #     oracle_critic_value1 = oracle_critic_values[0].squeeze(-1)  # (B,)
        #     oracle_critic_value2 = oracle_critic_values[1].squeeze(-1)  # (B,)

        # v1, v2 = self.oracles_values[oracle_name].get_value(data["observations"])  # shape = (num_qs, B, 1)
        # v1, v2 = v1.squeeze(-1), v2.squeeze(-1)  # (B,)
        # v_loss = F.mse_loss(v1, oracle_critic_value1, reduction="none").mean() + F.mse_loss(
        #     v2, oracle_critic_value2, reduction="none"
        # ).mean()
    ...
    def _compute_learner_critic_loss(self, data: dict):
        ...
        for _i, pred_info in enumerate(pred_v_infos):
            ...
            v_loss = v_loss + loss.mean()
        learner_critic_values = pred_v.mean()
        ...
        # ).mean()
        return q_loss, v_loss, q_mean, learner_critic_values, q_uncertainty
