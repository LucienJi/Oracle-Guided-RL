...
class NoAdv(BaseAlgo):
    ...
    def score_action_obs(self, obs_rep: torch.Tensor, actions: torch.Tensor, action_candidates_index: torch.Tensor, if_greedy: bool = False):
        """
        Score candidate actions using ONLY the learner critic/value.

        obs_rep: (N, D) replicated observation (learner-normalized if enabled)
        actions: (N, A) action candidates (typically proposed by oracle actors)
        action_candidates_index: kept for call-site compatibility (not used)
        if_greedy: if True, do not apply optimism term
        """
        q_mean, q_std, v = self._get_learner_training_critic(obs_rep, actions)  # (N,)
        if bool(if_greedy):
            return q_mean - v
        return (q_mean + float(self.kappa) * q_std) - v
    ...
    def prepare_action_candidates_obs(self, learner_obs_t: torch.Tensor, oracle_obs_t: torch.Tensor, k: int = 3):
        ...
        return: (N, A), action candidates from learner + oracles,
                shape is ((N_oracles + 1) * k * B, A) if B>1
        """
        cand_norm = []
        cand_final = []
        cand_index = []

        # Always include learner proposals (index 0)
        ...
        # Add oracle proposals (indices 1..n_oracles)
        for i in range(int(self.n_oracles)):
    ...
    def get_guided_training_action(self,raw_obs):
        ...
        selected_candidate_index = int(sampling_infos["env/selected_action_index"])
        policy_index = int(action_candidates_index[selected_candidate_index].item())
        sampling_infos["env/selected_candidate_index"] = float(selected_candidate_index)
        sampling_infos["env/selected_action_index"] = float(policy_index)
        ...
        while self.global_step < self.total_timesteps:
            ...
            with torch.inference_mode():
                if self.global_step < self.total_explore_steps:
                    ...
                    # NoAdv baseline: only use oracle actors during warmup.
                    actor_index = 0 if self.n_oracles <= 0 else (1 + int(actor_cycle_step % int(self.n_oracles)))
    ...
    def update(self):
        """Learner-only training (no oracle critic/value networks)."""
        self.learner_actor.train()
        self.learner_critic.train()
        self.learner_value.train()
        for name in self.oracles_names:
            # Oracles are fixed policies used for behavior / candidate generation.
        ...
        # --- Critic/value dataset: standard mixed replay ---
        critic_data = self._prepare_sampled_batch(
            self.replay_buffer.sample(int(self.args["learner_critic_batch_size"]))
        )
        critic_mini_batches = reshape_into_blocks(critic_data, num_blocks)

        info = {}
        for i in range(num_blocks):
            self.update_step += 1
            # Each mini_batches is a dict of tensors, shaped (num_blocks, ...)
            critic_sub_batch = {k: v[i] for k, v in critic_mini_batches.items()}
            critic_info = self.update_critic(critic_sub_batch)
    ...
    def update_critic(self, learner_data: dict):
        ...
        q_loss, v_loss, info = self._compute_critic_loss(learner_data)
    ...
    def _compute_critic_loss(self, learner_data: dict):
        ## learner part 
        info = {}
        total_q_loss = 0.0
        total_v_loss = 0.0
    ...
    def _compute_learner_critic_loss(self, data: dict):
        ...
        # Expectile-style value regression (kept as-is)
        # with torch.no_grad():
        #     actions = self.learner_actor.get_target_action(data["observations"], self.target_policy_noise)
        #     learner_critic_values = self.learner_critic.get_target_value(data["observations"], actions)  # (num_qs, B, 1)
        #     learner_critic_value1 = learner_critic_values[0].squeeze(-1)  # (B,)
        #     learner_critic_value2 = learner_critic_values[1].squeeze(-1)  # (B,)

        # v_values = self.learner_value.get_value(data["observations"])  # (num_qs, B, 1)
        # v1, v2 = v_values[0].squeeze(-1), v_values[1].squeeze(-1)  # (B,), (B,)
        # v_loss = F.mse_loss(v1, learner_critic_value1, reduction="none").mean() + F.mse_loss(
        #     v2, learner_critic_value2, reduction="none"
        # ).mean()
    ...
    def _compute_learner_actor_loss(self, data: dict):
        ...
        with torch.no_grad():
            ...
            weights = torch.exp((self.beta * advantage).clamp(-50.0, 10.0))
