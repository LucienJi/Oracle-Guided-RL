...
class CurrMaxAdv(BaseAlgo):
    def __init__(
        ...
        self.action_selection_buffer = deque(maxlen=int(max(self.learner_learning_starts + 500, self.args["action_selection_buffer_size"])))
        ...
        self.use_compile = bool(self.args.get("use_compile", True))
    ...
    def _try_compile(self, fn, name: str, *, fullgraph: bool = True):
        try:
            ...
            try:
                ...
                if bool(self.args.get("dynamo_suppress_errors", False)):
    ...
    def run(self, env, replay_buffer: ReplayBuffer, eval_env=None):
        ...
        while self.global_step < self.total_timesteps:
            ...
            # if self.use_wandb and isinstance(infos, dict) and "episode" in infos and self.global_step > self.learner_learning_starts:
            #     wandb.log(
            ...
            if self.global_step == self.learner_learning_starts:
                for _ in range(self.args['warmup_update_times']):
    ...
    def _compute_oracles_critic_loss(self, data: dict,oracle_name:str):
        # Keep this torch.compile-friendly: avoid Python-side shape casts; use tensor shapes directly.
        k = 20
        actor = self.oracles_actors[oracle_name]
        ...
        with torch.no_grad():
            ...
            # Multi-sample oracle actions -> average target critic distribution in probability space.
            # Oracle policies may own their own observation normalizer; prefer raw tensors when available.
            next_obs_for_oracle = data.get("next_observations_raw")
            b = data["next_observations"].shape[0]
            actions_k = actor.sample_action(next_obs_for_oracle, std, k)  # (k, B, A)
            actions_flat = actions_k.reshape(k * b, -1)  # (k*B, A)
            next_obs_rep = data["next_observations"].unsqueeze(0).expand(k, b, -1).reshape(k * b, -1)  # (k*B, D)
            next_qs, next_q_infos = critic.get_target_value_with_info(next_obs_rep, actions_flat)  # (num_qs, k*B, 1)
            min_idx = next_qs.squeeze(-1).argmin(dim=0)  # (k*B,)
            stacked = torch.stack([info["log_prob"] for info in next_q_infos], dim=0)  # (num_qs, k*B, num_bins)
            lp = stacked[min_idx, torch.arange(min_idx.shape[0], device=self.device)]  # (k*B, num_bins)
            next_q_log_probs = torch.logsumexp(lp.reshape(k, b, -1), dim=0) - math.log(k)  # (B, num_bins)
        ...
        with torch.no_grad():
            ...
            actions_k = actor.sample_action(obs_for_oracle, std, k)  # (k, B, A)
            actions_flat = actions_k.reshape(k * b, -1)  # (k*B, A)
            obs_rep = data["observations"].unsqueeze(0).expand(k, b, -1).reshape(k * b, -1)  # (k*B, D)
            current_qs, current_q_infos = critic.get_target_value_with_info(obs_rep, actions_flat)  # (num_qs, k*B, 1)
            min_idx = current_qs.squeeze(-1).argmin(dim=0)  # (k*B,)
            stacked = torch.stack([info["log_prob"] for info in current_q_infos], dim=0)  # (num_qs, k*B, num_bins)
            lp = stacked[min_idx, torch.arange(min_idx.shape[0], device=self.device)]  # (k*B, num_bins)
            current_q_log_probs = torch.logsumexp(lp.reshape(k, b, -1), dim=0) - math.log(k)  # (B, num_bins)
    ...
    def _compute_learner_actor_loss(self, data: dict):
        """
        - Guided action selection for imitation learning
        1. For each state, generate action proposals from learner and oracles
        2. Score all proposals using critics
        3. Select the highest-scoring action for each state
        4. Perform imitation learning to match the selected actions
        5. Also include TD3 RL loss term
        """
        obs = data["observations"]  # (B, D)
        # Get raw observations for oracles (if normalization is used)
        obs_raw = data.get("observations_raw", obs)  # (B, D)
        obs_learner = obs  # Already normalized if normalization is enabled
        
        B = obs.shape[0]
        eps = 1e-8
        
        # Generate action candidates for each state in the batch
        # For batch processing, we need to handle this carefully
        # Option: process each state individually or batch process all candidates together
        # We'll batch process: generate k proposals per policy for all states
        k = int(self.num_proposals)
        
        with torch.no_grad():
            # Generate action candidates: (N, A) where N = (n_oracles + 1) * k * B
            # The order is: [learner_k_actions for all states, oracle0_k_actions for all states, ...]
            final_action_candidates, action_candidates, action_candidates_index = self.prepare_action_candidates_obs(
                obs_learner, obs_raw, k
            )  # final: (N, A), norm: (N, A), index: (N,)
            
            # Score all action candidates
            n_candidates = action_candidates.shape[0]
            obs_rep = obs_learner.repeat(n_candidates // B, 1)  # (N, D) where N = (n_oracles + 1) * k * B
            scores, best_critic_index = self.score_action_obs(
                obs_rep,
                action_candidates,
                action_candidates_index,
                if_greedy=self.baseline_greedy_guided,
            )  # scores: (N,), best_critic_index: (N,)
            
            # Reshape to group by state: actions are arranged as [policy0_all_states_k, policy1_all_states_k, ...]
            # Each policy has k*B actions: [state0_k1, state0_k2, ..., state0_kk, state1_k1, ..., stateB-1_kk]
            # We need to reorganize to [state0_all_policies_k, state1_all_policies_k, ...]
            n_policies = self.n_oracles + 1
            # First reshape to (n_policies, k*B, A), then to (n_policies, k, B, A), then permute to (B, n_policies, k, A)
            action_candidates_reshaped = action_candidates.reshape(n_policies, k * B, -1).reshape(n_policies, k, B, -1).permute(2, 0, 1, 3).reshape(B, n_policies * k, -1)
            final_action_candidates_reshaped = final_action_candidates.reshape(n_policies, k * B, -1).reshape(n_policies, k, B, -1).permute(2, 0, 1, 3).reshape(B, n_policies * k, -1)
            scores_reshaped = scores.reshape(n_policies, k * B).reshape(n_policies, k, B).permute(2, 0, 1).reshape(B, n_policies * k)  # (B, n_policies * k)
            
            # Select highest-scoring action for each state
            best_indices = scores_reshaped.argmax(dim=1)  # (B,)
            selected_actions = action_candidates_reshaped[torch.arange(B, device=self.device), best_indices]  # (B, A)
            selected_final_actions = final_action_candidates_reshaped[torch.arange(B, device=self.device), best_indices]  # (B, A)
            
            # Compute advantage for selected actions (for weighting)
            qs = self.learner_critic.get_value(obs, selected_actions)  # (num_qs, B, 1)
            q_max = qs.mean(0).squeeze(-1)  # (B,)
            vs = self.learner_value.get_value(obs)  # (num_qs, B, 1)
            v_max = vs.mean(0).squeeze(-1)  # (B,)
            for critic in self._oracle_critic_list:
                qk = critic.get_value(obs, selected_actions).squeeze(-1).mean(dim=0)
                q_max = torch.maximum(q_max, qk)
            for vnet in self._oracle_value_list:
                vk = vnet.get_value(obs).squeeze(-1).mean(dim=0)
        ...
        # Normalize weights over the positive-advantage subset
        pos_count = pos_mask_f.sum()
        mean_weight = weights.sum() / (pos_count + eps)
        normalized_weights = weights / (mean_weight + eps)  # (B,)
        
        # Imitation learning: learner actor should match selected actions
        predicted_action = self.learner_actor.get_eval_action(obs)  # (B, A)
        raw_loss = (predicted_action - selected_actions.detach()).pow(2).sum(dim=-1)  # (B,)
        awbc_loss = (normalized_weights.detach() * raw_loss).sum() / (pos_count + eps)

        # Neutral TD3 term
        a_pi = self.learner_actor.get_eval_action(obs)  # (B, A)
        q_pi = self.learner_critic.get_value(obs, a_pi)  # (num_qs, B, 1)
        q_pi = q_pi.mean(0).squeeze(-1)  # (B,)
        td3_obj = (-1.0 * q_pi).mean()
        
        # Reweight AWBC loss based on TD3 objective magnitude
        awbc_loss = awbc_loss * (td3_obj.detach().abs()) / (awbc_loss.detach().abs() + eps)
