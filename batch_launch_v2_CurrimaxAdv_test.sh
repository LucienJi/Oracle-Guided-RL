...
export TIME="1:00:00"
export JOB_NAME_PREFIX="CurrimaxAdv_test"
...
for task in "${tasks[@]}"; do
    ...
    echo "Submitting CurrimaxAdv_test job 1/3 (seeds $SEEDS_GROUP1) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "train_CurrimaxAdv_test" "$SEGMENTS" "$SEEDS_GROUP1" \
        --config-name "metaworld/CurrimaxAdv_${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting CurrimaxAdv_test job 2/3 (seeds $SEEDS_GROUP2) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "train_CurrimaxAdv_test" "$SEGMENTS" "$SEEDS_GROUP2" \
        --config-name "metaworld/CurrimaxAdv_${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting CurrimaxAdv_test job 3/3 (seeds $SEEDS_GROUP3) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "train_CurrimaxAdv_test" "$SEGMENTS" "$SEEDS_GROUP3" \
        --config-name "metaworld/CurrimaxAdv_${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
done
