...
class SawyerDisassembleOraclePolicy(TorchBaseOraclePolicy):
    @staticmethod
    ...
    def _desired_pos(self, o_d: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pos_wrench = o_d["wrench_pos"] + torch.tensor([-0.02, 0.0, 0.01], device=device, dtype=dtype)

        desired = pos_curr + torch.tensor([0.0, 0.0, 0.1], device=device, dtype=dtype)
        ...
        remaining = ~cond0
        cond1 = torch.abs(pos_curr[:, 2] - pos_wrench[:, 2]) > 0.03
        mask = remaining & cond1
        desired = torch.where(mask.unsqueeze(-1), pos_wrench, desired)
    ...
    def _grab_effort(o_d: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        ) | (torch.abs(pos_curr[:, 2] - pos_wrench[:, 2]) > 0.07)
