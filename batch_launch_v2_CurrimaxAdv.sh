...
for task in "${tasks[@]}"; do
    ...
    echo "Submitting CurrimaxAdv job 1/3 (seeds $SEEDS_GROUP1) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "train_CurrimaxAdv" "$SEGMENTS" "$SEEDS_GROUP1" \
        --config-name "metaworld/CurrimaxAdv_${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting CurrimaxAdv job 2/3 (seeds $SEEDS_GROUP2) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "train_CurrimaxAdv" "$SEGMENTS" "$SEEDS_GROUP2" \
        --config-name "metaworld/CurrimaxAdv_${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting CurrimaxAdv job 3/3 (seeds $SEEDS_GROUP3) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "train_CurrimaxAdv" "$SEGMENTS" "$SEEDS_GROUP3" \
        --config-name "metaworld/CurrimaxAdv_${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
done

echo "Batch submission complete (CurrimaxAdv)."
squeue -u "$(whoami)"
