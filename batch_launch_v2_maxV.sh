...
for task in "${tasks[@]}"; do
    EXTRA_ARGS=(method_name=maxV "training.total_timesteps=${TOTAL_TIMESTEPS}" "training.capacity=${CAPACITY}")
    [[ " $SPARSE_TASKS " == *" $task "* ]] && EXTRA_ARGS+=(env.sparse_reward=true)

    echo "Submitting maxV job 1/2 (seeds $SEEDS_GROUP1) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "baselines.train_maxVQ" "$SEGMENTS" "$SEEDS_GROUP1" \
        --config-name "baselines_configs/maxVQ/metaworld/${task}" \
        "${EXCLUDE_OPT[@]}" "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting maxV job 2/2 (seeds $SEEDS_GROUP2) for task: $task"
    bash "$LAUNCH_SCRIPT" \
...
echo "Batch submission complete (maxV)."
squeue -u "$(whoami)"
