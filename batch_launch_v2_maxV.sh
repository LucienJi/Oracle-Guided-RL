...
for task in "${tasks[@]}"; do
    ...
    echo "Submitting maxV job 1/3 (seeds $SEEDS_GROUP1) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "baselines.train_maxVQ" "$SEGMENTS" "$SEEDS_GROUP1" \
        --config-name "baselines_configs/maxVQ/metaworld/${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting maxV job 2/3 (seeds $SEEDS_GROUP2) for task: $task"
    bash "$LAUNCH_SCRIPT" \
        "baselines.train_maxVQ" "$SEGMENTS" "$SEEDS_GROUP2" \
        --config-name "baselines_configs/maxVQ/metaworld/${task}" \
        "${EXTRA_ARGS[@]}"
    sleep 1
    echo "Submitting maxV job 3/3 (seeds $SEEDS_GROUP3) for task: $task"
    bash "$LAUNCH_SCRIPT" \
