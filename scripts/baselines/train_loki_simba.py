...
@hydra.main(config_path="../../config", config_name="baselines_configs/loki_quad", version_base=None)
def train_loki_simba(cfg: DictConfig):
    ...
    # Oracles (actors, Simba checkpoints)
    ...
    if len(oracles_dict) == 0:
        raise ValueError("LOKI requires oracles_dict to be specified in the config.")
    ...
    oracles_actors, _, _ = build_simba_oracle_modules_from_cfg(
        cfg=cfg,
        obs_shape=obs_shape,
        act_dim=act_dim,
        device=device,
    ...
    if "k_min" in args and "loki_N_min" not in args:
        args["loki_N_min"] = args["k_min"]
    if "k_max" in args and "loki_N_max" not in args:
        args["loki_N_max"] = args["k_max"]
    ...
    # Train
    print("Starting training (LOKI Simba)...")
